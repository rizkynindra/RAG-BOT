...
def main():
    st.title("🧑‍💻 BPJSTK 💬 Assistant V1.0.0")
    # """
    # My name is Marco, your personal AI Assistant. I create file_search assistants, just upload your knowledge base and start chatting to your documents.
    # """

    if 'assistant_initialized' not in st.session_state:
        # Input field for the title
        # title = st.text_input("Enter the title", key="title")
        initiation = st.text_input("Halo, silakan tuliskan pertanyaanmu", key="initiation")

        vector_key = os.getenv("VECTOR_STORE_ID")
        thread_id = startAssistantThread(initiation, vector_key)
        ...
        # st.write("Assistant ID:", assist_id)
        # st.write("Vector ID:", vector_key)
        # st.write("Thread ID:", thread_id)

        process_run(st, thread_id, assist_id)
        ...
        #         assistant_id, vector_id = createAssistant(file_ids, title)
            ...
            # Start the Thread
            # thread_id = startAssistantThread(initiation, vector_id=vec)
            #
            # # Save state
            # st.session_state.thread_id = thread_id
